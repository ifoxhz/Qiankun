Qiankun::Admin.controllers :organizes do  
  ...
  delete :destroy_many do
    @title = "Organizes"
    unless params[:organize_ids]
      flash[:error] = pat(:destroy_many_error, :model => 'organize')
      redirect(url(:organizes, :index))
    end
    ids = params[:organize_ids].split(',').map(&:strip)
    organizes = Organize.find(ids)
    
    if Organize.destroy organizes
    
      flash[:success] = pat(:destroy_many_success, :model => 'Organizes', :ids => "#{ids.to_sentence}")
    end
    redirect url(:organizes, :index)
  end
end
